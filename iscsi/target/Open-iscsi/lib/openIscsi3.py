...
def sshCmd(ip, user, cmd):
    ...
    osCmd = '''ssh -i  ~/.ssh/key %s@%s "python /root/nbs-environ/0.6/VolumeServer/lib/sendtarget0.py %s"'''%(user,ip,cmd)
    runExec(osCmd)
#DEBUG=True
DEBUG=False
if __name__ == "__main__":
    if DEBUG:
        pdb.set_trace()
    while True:
        sshCmd("10.120.37.8","root","send")
        print "send target success" 
        loginIscsiTarget("loop0")
        print "login success" 
        time.sleep(2)
        logoutIscsiTarget("loop0")
        print "logout success" 
        sshCmd("10.120.37.8","root","purge")
