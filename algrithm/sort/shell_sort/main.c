...
void insertion_sort1(int arr[], int len)
{
	...
	for (int i = 1; i < len; i++)
	{
		...
		for (; j>=1&&arr[j-1]>temp; j--)    // 写成 j>=1 容易跟shell排序对比
		{
...
void insertion_sort(int arr[], int len)
{
	...
	for (int i = 1; i < len; i++)
	{
		...
		{
			arr[j+1] = arr[j];  // a[j-1] 比temp要大 所以需要移动
		}
		arr[++j] = temp;
	}
}
...
{
	for (int inc = len/2; inc >=1; inc = inc/2)
	{
		// 对i 从inc 到len-1 排序
		for (int i = inc; i < len; i++)
		{
			...
			{
				arr[j] = arr[j-inc];  // a[j-1] 比temp要大 所以需要移动
			}
			arr[j] = temp;
		}
	}
...
void shell_sort(int arr[], int len)
{
	...
	for (int inc = len/2; inc >=1; inc = inc/2)
	{
		for (int i = inc; i < len; i++)
		{
			...
			{
				arr[j+inc] = arr[j];  // a[j-1] 比temp要大 所以需要移动
			}
			arr[j+inc] = temp;
		}
	}
