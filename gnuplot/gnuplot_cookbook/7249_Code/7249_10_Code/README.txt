...
recipe. You can load them into gnuplot at the interactive prompt with the
"load" command or execute them on the system command line with "gnuplot
--persist FILE"; substitute the name of the code file for FILE. The "--persist"
option will cause the graph to remain on the screen after gnuplot exits.
...
Which data goes with which recipe is explained in the chapter text. You will
 need to be in the same directory as the data file to execute the script that 
