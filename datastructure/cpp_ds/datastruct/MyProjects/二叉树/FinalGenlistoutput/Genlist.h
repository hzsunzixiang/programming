...
GenListNode *GenList ::CreateList ( GenListNode *ls,  char * s ) {
  ...
  {ls->tlink = NULL;}	          //空表	
     ...
     GenListNode* p = ls;
	  while ( strlen (sub)!= 0) {   //建立表结点
           ...
           //创建新结点,向表尾链接
		   cout<<"the address of p   "<<p<<"  the address of p"<<endl;
		   cout<<"the address of222*ls  "<<ls<<"  The address of *ls"<<endl;
           if ( sever (sub, hsub) ) {
           //以逗号为界,分离第一个表元素hsub
              if ( hsub[0] != '('&& hsub[0] !='\'' ) {
	      //非子表,非字符分界符
				 ...
				 cout<<"IN THE FUNCTION THE VALUE OF p->value.intgrinfo   "<<p->value.intgrinfo<<"  FUNCITON";
              } 
              else if ( hsub[0] != '(' && hsub[0] == '\'' ) {
              //非子表且是字符分界符
				  p->utype = CH;  
				  p->value.charinfo = hsub[1];	
				  cout<<"IN THE FUNCTION THE VALUE OF char p->value.intgrinfo    "<<p->value.charinfo<<"     FUNCITON";
               }
               else {                 //是子表,递归建立子表
                  p->utype = LST; 
				  cout<<"IN THE lst";
				  ...
				  cout<<"the 内部1111the value of I "<<i<<"the length of hsub"<<strlen(hsub)<<"  the type of utype  "<<p->utype<<"  the value of I"<<endl;
				  cout<<"the address of return of Creatlist  "<<CreateList ( p->value.hlink, hsub )<<"  CreatList";
				  
               }
			   	  cout<<"the 内部22222the value of I "<<i<<"  the type of  utype "<<p->utype<<"  the value of I"<<endl;
		   }
            else return NULL;     //字符串表达式有错
			
			cout<<"the address of111111*ls  "<<ls<<"The address of *ls"<<endl;
  ...
  cout<<"the final address of ls"<<ls<<"the final"<<endl;
  cout<<"the 外部the value of I "<<i<<"  the value of I"<<endl;
  cout<<"the final  address of ls->tlink"<<ls->tlink<<endl;
    return ls;
}

void GenList ::output (GenListNode *ls){
	cout<<"the is the address of *ls!"<<ls<<endl;
     if ( ls!= NULL ) {
		switch ( ls->utype ) {//根据utype来传送值域	
           case HEAD: 
           ...
           case LST: 
			   ...
			   cout<<"THIS IS THE ADDRESS OF ls->value.hlink"<<ls->value.hlink<<endl;
			   GenListNode *temp;
			   temp=ls->value.hlink;
