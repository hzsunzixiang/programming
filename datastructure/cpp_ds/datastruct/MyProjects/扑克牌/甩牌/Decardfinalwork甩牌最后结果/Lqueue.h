...
template <class Type> void Queue<Type>::DeCard(Queue<Type> &queue) {//建立一队列，以便后面操作
	cout<<endl<<endl;   //此时显示的是：每张牌（牌依次递增）所在的位置应该是多少
	                    //可以看出，此时很不直观，所以下面有result()函数来改进
	int count=1;
	Type temp1;
	int temp2;
	while(!IsEmpty() ){
...
//此时显示的是在每个位置（位置依次递增）应该是哪张牌
	cout<<"location    "<<"     card"<<endl;
	Type temp1;
	int count=1,temp2;   //count为每个位置的标记，count递增
	while(!IsEmpty() ){//依次取出元素，直到队列空
		if(GetFront_data()==count){//如果满足则出对
			cout<<GetFront_data()<<"                 "<<GetFront_flag()<<endl;
			DeQueue();count++;
		}
		else{	                  //否则出队之后再入队
