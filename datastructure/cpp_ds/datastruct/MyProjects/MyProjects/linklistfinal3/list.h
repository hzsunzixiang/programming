...
template <class Type> void List<Type>::Inverse( ) {
	ListNode<Type> *pre,*cur,*temp ;
	pre=first;cur=pre->link;
	if(cur==NULL) {cout<<"empty!"<<endl;}
	else {
		ListNode<Type> *next;
		next=cur->link;
		{
			if(next==NULL)  {cout<<"only one node!"<<endl;}
				else{
...
template<class Type> void List<Type>::Combine(List<Type> &hb){
	ListNode<Type> *pa,*pb,*p,*q;
