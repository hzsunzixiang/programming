...
void do_service(int connfd)
{
    ...
    while(1)
    {
        ...
        int ret = readline(connfd, recvbuf, sizeof(recvbuf)); // 从客户端收取数据
        fputs(recvbuf, stderr); // 把结果打印出来
        if (ret > 0)
        {
            if (sscanf(recvbuf, "%ld%ld", &args1, &args2) == 2)
            {
                snprintf(recvbuf, sizeof(recvbuf), "%ld\n", args1 + args2);
            }
            else
                snprintf(recvbuf, sizeof(recvbuf), "input error\n");
            
			if ((ret = write(connfd, recvbuf, strlen(recvbuf))) < 0)
			{
