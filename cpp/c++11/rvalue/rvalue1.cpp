...
int main()
{
	...
	// 这样就是错误的  因为p1 是个lvalue
	//Point3d && p2 = p1;

	// 但是这样则是对的 这个很意外
	auto&& p4 = p1; // here, "&&" does not mean rvalue reference


	// 此时的等价 为 Point3d &p3 = p2 
	auto&& p3 = p2;

	//报错: cannot bind ‘Point3d’ lvalue to ‘Point3d&&’
	//此时 p2 是可取地址的，不是一个rvalue, 而是lvalue
	//Point3d && p5 = p2;
