def action(url='http://api.qcloud.com/v2', inquiryable=False, switchable=False, inquiryresourceable=False, **args):
    def _(func):
        ...
        if inquiryresourceable:
            supported_actions[get_inquiry_resource_action_name_prefix() + func.name] = func
        if switchable:
        ...
        func.url = url
        assert args.get('desc'), "%s API 没有通过 Action 增加描述。" % func.name
        func.desc = args['desc']
        debug('识别到 %s 入口点并挂载...' % func.name)
