...
class BaseAdapter(object):
    ...
    def __init__(self):
        ...
        print("######getImagesDetail:%s"% self.getImagesDetail)
        print("######getImagesDetail:%s"% self.__class__.__dict__["getImagesDetail"])
        ...
        for attr in list(self.__class__.__dict__.keys()):
            ...
            if (attr not in keyword_set and '_' != attr[0]) and callable(self.__getattribute__(attr)):
                print("-----attr:%s-----begin"%attr)
                xx = self.__getattr__(attr)
                print("-----getattrhhh:%s-----"%xx)
                self.__setattr__(attr, xx)
                # 等价于下面这句
                #self.__setattr__(attr, self.__getattr__(attr))
                print("-----attr:%s----- end"%attr)
        print(self.__class__.__dict__)
        print("######getImagesDetail:%s"% self.getImagesDetail)
        print("######getImagesDetail:%s"% self.__dict__["getImagesDetail"])
