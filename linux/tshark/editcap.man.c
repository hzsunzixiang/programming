...
SYNOPSIS
       editcap [ -A <start time> ] [ -B <stop time> ] [ -c <packets per file> ] [ -C [offset:]<choplen> ] [ -E <error probability> ] [ -F <file format> ] [ -h ]
       [ -i <seconds per file> ] [ -L ] [ -r ] [ -s <snaplen> ] [ -S <strict time adjustment> ] [ -t <time adjustment> ] [ -T <encapsulation type> ] [ -v ] infile outfile
       [ packet#[-packet#] ... ]
...
OPTIONS
       ...
       -C  [offset:]<choplen>
           ...
           If an optional offset precedes the <choplen>, then the bytes chopped will be offset from that value. Positve offsets are from the packet beginning, while negative offsets are
           from the packet end.
       ...
       -S  <strict time adjustment>
           ...
           If <strict time adjustment> value is specified as a negative value, then the timestamp values of all packets will be adjusted to be equal to the timestamp value of the
       ...
       -w  <dup time window>
           ...
           NOTE: The -w option assumes that the packets are in chronological order.  If the packets are NOT in chronological order then the -w duplication removal option may not
...
NOTES
       Editcap is part of the Wireshark distribution.  The latest version of Wireshark can be found at <http://www.wireshark.org>.
