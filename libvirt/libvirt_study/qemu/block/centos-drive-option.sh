...
-smp 2 -boot c \
-drive file=/vm/centos7hda_from_qemu,if=ide,index=0,media=disk,format=raw \
-drive file=/vm/data_disk.img,if=ide,index=1,media=disk,format=raw \
-drive file=/vm/data_disk2.img,if=virtio,index=2,media=disk,format=qcow2 \
-drive file=/vm/data_disk3.img,if=virtio,index=3,media=disk,format=qcow2 \
-drive file=/vm/data_disk4.img,if=virtio,index=4,media=disk,format=qcow2 \
-drive file=/root/CentOS-7-x86_64-Minimal-2009.iso,media=cdrom \
-netdev bridge,br=virbr0,id=n4 -device virtio-net,netdev=n4 \
-vnc 0.0.0.0:7 -k en-us 


# 加上这个起不来
# -drive file=/dev/sdc,if=virtio,media=disk,format=raw \
