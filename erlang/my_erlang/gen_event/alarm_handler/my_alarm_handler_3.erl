...
handle_event({set_alarm, {AlarmId, EventDesc}}, Alarms) ->
    error_logger:error_msg("*** Tell the Engineer to turn on the fan AlarmId: ~p,~p~n", [AlarmId, EventDesc]),
    case lists:keyfind(AlarmId, 1, Alarms) of 
        {Type, Num} -> 
            NewAlarms = lists:keyreplace(Type, 1, Alarms, {Type, Num+1});
        false ->
            NewAlarms = lists:append([{AlarmId, 1}], Alarms)
    end,
    {ok, NewAlarms};  %%%%% 注意这里是分号，不然会报错
...
handle_event({clear_alarm, AlarmId}, Alarms) ->
    ...
    case lists:keyfind(AlarmId, 1, Alarms) of 
        {Type, Num} -> 
               if Num =< 1 -> NewAlarms =  lists:keydelete(Type, 1, Alarms);
                  true -> NewAlarms = lists:keyreplace(Type, 1, Alarms, {Type, Num-1})
               end;
        false ->
             NewAlarms = Alarms 
    end,
...
start() ->
    ...
    gen_event:swap_handler(alarm_handler, {alarm_handler, swap}, {my_alarm_handler_3, []}),
    ...
    alarm_handler:set_alarm({100,tooHot}),
    alarm_handler:set_alarm({100,tooHot}),
    alarm_handler:set_alarm({101,tooCold}),
    alarm_handler:set_alarm({101,tooCold}),
    alarm_handler:clear_alarm(100),
    alarm_handler:clear_alarm(100),
    alarm_handler:clear_alarm(101),
    alarm_handler:clear_alarm(101),
    alarm_handler:clear_alarm(101),
    'this is an end'.
...
%2> alarm_handler:set_alarm({100,tooHot}).
%=INFO REPORT==== 10-Mar-2023::02:42:16.470778 ===
%    alarm_handler: {set,{100,tooHot}}
%ok
%3> alarm_handler:set_alarm({101,tooCold}).
%ok
%4> =INFO REPORT==== 10-Mar-2023::02:42:19.179332 ===
%    alarm_handler: {set,{101,tooCold}}
%gen_event:swap_handler(alarm_handler, {alarm_handler, swap}, {my_alarm_handler_3, []}).
%in init here call  {Args:[], {alarm_handler, Alarms:[{101,tooCold},
%                                                     {100,tooHot}]}}
%ok
%5> alarm_handler:set_alarm({100,tooHot}).
%ok
%6> =ERROR REPORT==== 10-Mar-2023::02:42:28.100502 ===
%*** Tell the Engineer to turn on the fan AlarmId: 100,tooHot
%
%
%6> alarm_handler:set_alarm({101,tooCold}).
%ok
%7> =ERROR REPORT==== 10-Mar-2023::02:42:51.413655 ===
%*** Tell the Engineer to turn on the fan AlarmId: 101,tooCold
%
%..... 删除
%
%
%13> alarm_handler:clear_alarm(100).
%ok
%14> =ERROR REPORT==== 10-Mar-2023::02:44:54.019055 ===
%*** Danger over. Turn off the fan AlarmId:100
%
%
%14> alarm_handler:clear_alarm(101).
%ok
%15> =ERROR REPORT==== 10-Mar-2023::02:45:11.162186 ===
%*** Danger over. Turn off the fan AlarmId:101
%
%
%15> alarm_handler:clear_alarm(101).
%ok
%16> =ERROR REPORT==== 10-Mar-2023::02:45:29.146509 ===
%*** Danger over. Turn off the fan AlarmId:101
%
%
%
