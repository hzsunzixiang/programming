...
start() ->
    ...
    logger:update_handler_config(default, #{filter_default=>log, formatter=>{rabbit_logger_text_fmt,#{single_line => false,use_colors => false}}}),

    LogFile = "log/mq1.log",
    logger:add_handler(rmq_1_file_1, rabbit_logger_std_h, #{id=>rmq_1_file_1, module=>rabbit_logger_std_h, config=>#{type=>file, file=>"log/mq1.log", burst_limit_enable=>false}, level=>debug, filter_default=>log, filters=>[{progress_reports,{fun logger_filters:progress/2,log}}, {rmqlog_filter, {fun filter_log_event/2, #{global=> all,prelaunch => notice}}}], formatter=>{rabbit_logger_text_fmt,#{single_line => false,use_colors => false}}}),
	...
	% #{domain => ?RMQLOG_DOMAIN_GLOBAL} 这里就是metadata，匹配 filter_log_event 中的meta参数
