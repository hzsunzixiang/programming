...
sed 's/erts_version/'"$ERTS_VERSION"'/g;s/kernel_version/'"$KERNEL_VERSION"'/g; s/stdlib_version/'"$STDLIB_VERSION"'/g; s/sasl_version/'"$SASL_VERSION"'/g' basestation.rel.template > basestation.rel
...
RUN_DIR_NAME=`pwd`/ernie
sed 's#RUN_DIR_NAME#'"$RUN_DIR_NAME"'#g' bsc_heart.template > bsc_heart
